...
stages = [{'stage_mass': 38051, 'fuel_mass': 97000, 'f_tract': 2964000, 'burn_time': 90},
          {'stage_mass': 15325, 'fuel_mass': 20123, 'f_tract': 704000, 'burn_time': 60}]
...
def corner(altitude): #вычисление угла гравитационного поворота
    if altitude < 1000: return 90
    elif altitude <= 30000: return 90 * (1 - altitude / 60000)
    elif altitude <= 100000: return 45 * (1 - altitude / 100000)
    else: return 0
...
time_first = numpy.linspace(0, 90) #время работы первой ступени
...
time_second = numpy.linspace(90, 150, 90) #время работы второй ступени
result_second = odeint(odeint_func, start_data_2, time_second, args=(k_second, f_tract_second, )) #результаты для второй ступени

new_mass_2 = result_second[-1].copy() #последний результат работы второй ступени
new_mass_2[-1] -= stages[1]['stage_mass'] #отделение второй ступени
start_data_3 = new_mass_2 #начало работы третьей ступени

time_third = numpy.linspace(150, 240, 90)
result_third = odeint(odeint_func, start_data_3, time_third, args=(0, 0, ))

#объединение результатов обеих ступеней
time = numpy.concatenate([time_first, time_second, time_third])
x = numpy.concatenate([result_first[:, 0], result_second[:, 0], result_third[:, 0]])
x_speeds = numpy.concatenate([result_first[:, 1], result_second[:, 1], result_third[:, 1]])
y = numpy.concatenate([result_first[:, 2], result_second[:, 2], result_third[:, 2]])
y_speeds = numpy.concatenate([result_first[:, 3], result_second[:, 3], result_third[:, 3]])
mass_rocket = numpy.concatenate([result_first[:, 4], result_second[:, 4], result_third[:, 4]])
...
y_speeds_ksp = numpy.array(data_ksp[0]["y_speeds"]) 
...
plt.title('Изменение высоты ракеты от времени')
plt.xlabel('Время, с')
plt.ylabel('Высота, м')
plt.legend()
plt.grid()
...
plt.title('Изменение скорости ракеты от времени')
plt.xlabel('Время, с')
...
plt.title('Изменение массы ракеты от времени')
plt.xlabel('Время, с')
plt.ylabel('Масса, кг')
plt.legend()
plt.grid()
...
plt.title('Изменение горизонтальной скорости ракеты от времени')
plt.xlabel('Время, с')
plt.ylabel('Скорость, м/с')
plt.legend()
plt.grid()
...
plt.plot(time_ksp, y_speeds_ksp, label='Вертикальная скорость по КСП', color='blue')
plt.title('Изменение вертикальной скорости ракеты от времени')
plt.xlabel('Время, с')
plt.ylabel('Скорость, м/с')
plt.legend()
plt.grid()
...
plt.plot(numpy.array([time_ksp[i] for i in range(0, len(time_ksp), 13)]), accelerations_ksp, label='Ускорение по КСП', color='blue')
plt.title('Изменение ускорения ракеты от времени')
plt.xlabel('Время, c')
